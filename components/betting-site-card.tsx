    ...
    >
      <div
        className={`relative rounded-xl p-6 transition-all duration-300 group-hover:scale-[1.02] shadow-xl border-2 ${
          isFirstItem
            ? "bg-gradient-to-br from-blue-100 via-white to-red-100 border-yellow-400 shadow-2xl shadow-yellow-400/30"
            : "bg-gradient-to-br from-blue-50 via-white to-red-50 border-slate-300 hover:border-slate-400"
        }`}
      >
        ...
        {/* Éléments décoratifs français */}
        <div className="absolute inset-0 rounded-xl overflow-hidden">
          {isFirstItem ? (
            <>
              {/* Étoiles françaises pour le premier */}
            ...
            <>
              {/* Motifs géométriques français pour les autres */}
              ...
              {/* Cercles décoratifs */}
              <div className="absolute -bottom-6 -left-6 w-20 h-20 rounded-full bg-gradient-to-br from-blue-200/15 to-blue-300/10"></div>
              <div className="absolute -top-6 -right-6 w-20 h-20 rounded-full bg-gradient-to-br from-red-200/15 to-red-300/10"></div>
            </>
          )}
        </div>
        ...
        <div className="relative z-10">
          ...
          <div className="hidden md:flex md:items-center md:gap-4 lg:gap-6 pt-2">
            ...
            <div className="flex-1 text-center">
              <div className={`font-bold mb-1 text-xl ${isFirstItem ? "text-orange-700" : "text-slate-700"}`}>
                BONUS INFORMATIONS
              </div>
              <p
                className={`font-semibold leading-tight text-2xl ${isFirstItem ? "text-orange-800" : "text-green-600"}`}
              >
                {site.bonus}
              </p>
            </div>
            ...
            <div className="text-center flex-shrink-0">
              <div
                className={`text-xl lg:text-2xl font-bold mb-1 ${isFirstItem ? "text-orange-700" : "text-slate-700"}`}
              >
            ...
            <div className="flex-shrink-0">
              <Button
                className={`font-bold px-6 py-3 text-base border-2 flex items-center gap-2 shadow-lg transition-all hover:scale-105 ${
                  isFirstItem
                    ? "bg-red-600 hover:bg-red-700 text-white font-bold border-orange-500"
                    : "bg-blue-600 hover:bg-blue-700 text-white font-bold border-blue-500"
                }`}
              >
                VISITER SITE
          ...
          <div className="md:hidden pt-2">
            ...
            <div className="flex items-start justify-between mb-4">
              ...
              <div className="flex-1 text-center pl-4">
                <div className={`font-bold text-sm mb-1 ${isFirstItem ? "text-orange-700" : "text-slate-700"}`}>
                  BONUS INFO
                </div>
                <p
                  className={`font-semibold leading-tight text-lg ${isFirstItem ? "text-orange-800" : "text-green-600"}`}
                >
                  {site.bonus}
                </p>
              </div>
            ...
            <div className="flex items-center justify-center gap-4 mb-4">
              <div className="flex items-center gap-2">
                <span className={`font-bold text-lg ${isFirstItem ? "text-orange-700" : "text-slate-700"}`}>
                  {site.rating.toFixed(1)}
                  <span className="text-sm text-slate-500">/10</span>
                </span>
